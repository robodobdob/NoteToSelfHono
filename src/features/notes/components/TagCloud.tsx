...
async function TagCloud() {
    ...
    return (
        <section id="tag-cloud">
            <div className="fw-bold text-uppercase mb-3">Filter by tag</div>
            <form className="d-flex justify-content-center flex-wrap mb-3 gap-2" hx-get="/notelist"
                  hx-indicator="#mainLayout_spinner" hx-target="#notes-list" hx-trigger="change">
                {tags.map(tag =>
                    <span>
                        <input type="checkbox" className="btn-check" value={tag.Name} name="Tags" id={tag.Name}
                               autoComplete="off"/>
